  ...
  return (
    ...
    >
      {(() => {
        ...
        return <IconComp size={size} color={colors.textPrimary} />;
      })()}
    </TouchableOpacity>
  );
}
    ...
    <View style={styles.switcherContainer}>
      {options.map((option) => {
        ...
        return (
          ...
          >
            {(() => {
              const IconComp = iconMap[option.icon] || Sun;
              return (
                <IconComp size={18} color={isSelected ? colors.surface : colors.textPrimary} />
              );
            })()}
            <Text
              style={[
