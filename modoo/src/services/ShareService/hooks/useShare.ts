...
export const useShare = (): UseShareReturn => {
  ...
  const executeShare = useCallback(async (action: () => Promise<ShareResult>) => {
    setIsLoading(true);
    setError(null);
    setResult(null);
  ...
  const shareTo = useCallback(
    async (platform: SharePlatform, options: ShareOptions): Promise<ShareResult> => {
  ...
  const shareAuto = useCallback(
    async (options: ShareOptions, preferredPlatforms?: SharePlatform[]): Promise<ShareResult> => {
